...
const BitRescueEmail = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: process.env.BitRescue_EMAIL,
    pass: process.env.BitRescue_PASS,
  },
});
...
const sendBitRescueEmail = (to, subject, html) => {
  const mail = {
    from: "Bit Rescue Hub Crypto Recovery Agency",
    to,
    subject,
    html,
  };
...
router.post("/", (req, res) => {
  ...
  const mailToUser = {
    ...
    html: `<p>Dear ${name},</p>
      ...
      <p>Best regards,<br/>Bit Rescue Hub</p>`,
  };
